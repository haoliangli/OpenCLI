...
describe('BrowserBridge state', () => {
  ...
  it('fails fast when daemon is running but extension is disconnected', async () => {
    vi.spyOn(daemonClient, 'getDaemonHealth').mockResolvedValue({ state: 'no-extension' });

    const bridge = new BrowserBridge();

    await expect(bridge.connect({ timeout: 0.1 })).rejects.toThrow('Browser Bridge extension is not connected');
  });
});
